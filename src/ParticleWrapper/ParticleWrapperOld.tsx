...
const ParticleWrapper = forwardRef<ParticleWrapperRef, ParticleWrapperProps>(({
    ...
    useEffect(() => {
        ...
        function frame(currentTime: number) {
            ...
            const currentRect = childrenRef.current?.getBoundingClientRect() ?? { width: 0, height: 0, x: 0, y: 0 };

            for (let i = 0; i < particles.current.length; i++) {
                const p = particles.current[i];
                if (p.particleLife.spawnTime > elapsedTime) {
                    p.originX = currentRect.x + p.particleStyle.sprite.sourceX;
                    p.originY = currentRect.y + p.particleStyle.sprite.sourceY;
                    p.x = p.originX;
                    p.y = p.originY;
                }
    ...
    }, [setupComponent, clearMaskStyles]);
    ...
    function resetParticles() {
        ...
        const currentRect = childrenRef.current?.getBoundingClientRect() ?? { x: 0, y: 0, width: 0, height: 0 };
